...
- **로고 이미지** — `assets/logo.png`가 비어 있습니다. `sections/00-nav.html`의 `<img>`가 이 파일을 참조하도록 바꿔뒀는데,
  원본 아임웹 위젯의 로고는 매우 긴 base64 텍스트라 자동으로 안전하게 옮길 수 없었습니다.
  아임웹 관리자 페이지의 미디어 라이브러리(또는 갖고 계신 원본 로고 파일)에서 다운받아 `assets/logo.png` 자리에 넣어주세요.
  (아임웹에 다시 붙여넣을 때는 파일 업로드 후 나오는 URL로 `src`를 바꾸거나, 원래처럼 base64로 인코딩해서 넣으면 됩니다.)
